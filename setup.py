...
setup(
    ...
    author="Dhavle YB",
    name="Billing system",
    options={'bulid_exe':{'include_files':includefiles},'bdist_msi':bdist_msi_option,},
    executables=[
        Executable(
            script="main.py",
            base=base,
...
def send_email():
    def send_gmail():
        try:
                    ...
                    reciver_address=reciverEntry.get()
                    ob.sendmail(senderEntry.get(),reciverEntry.get(),message)
                    ob.quit()
                    messagebox.showinfo('Success','Bill is succesfully sent',parent=root1)
                    root.destroy()
